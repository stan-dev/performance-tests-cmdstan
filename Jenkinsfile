...
def cleanCheckout() {
    ...
    checkout([$class: 'GitSCM',
        ...
        submoduleCfg: [],
        userRemoteConfigs: [[url: "git@github.com:stan-dev/performance-tests-cmdstan.git", credentialsId: 'a630aebc-6861-4e69-b497-fd7f496ec46b']]
    ])
                ...
                stage("Test cmdstan base against cmdstan pointer in this branch on windows") {
                    ...
                    steps {
                        ...
                        script{
                                ...
                                bat """
                                    bash -c "cd cmdstan && git submodule update --init --recursive"
                                    bash -c "cd cmdstan && git pull origin ${params.cmdstan_origin_pr}"
                                    bash -c "cd cmdstan && git submodule update --init --recursive"
                                """
